// Este componente grafica tres índices bursátiles (DAX, S&P 500, IBEX).
// Los datos son generados de manera simulada, ya que las APIs para obtener datos reales suelen ser de pago.
...
const StockIndicesDashboard = () => {
  // Estado para controlar el índice activo (DAX, S&P 500, IBEX)
  const [activeIndex, setActiveIndex] = useState('dax')
  // Estado para controlar el período de tiempo seleccionado (1 día, 5 días, etc.)
  ...
  // Lista de períodos de tiempo disponibles
  const periods = [
    { id: '1d', label: '1 día' },
    { id: '5d', label: '5 D' },
    { id: '1m', label: '1 mes' },
  ...
  // Efecto para cargar datos simulados al montar el componente
  useEffect(() => {
    const fetchIndexData = async () => {
      // Genera datos simulados para todos los índices y períodos
      const mockDataForAllIndices = generateMockDataForAllPeriods()
      
      // Simula una carga de datos con un retraso de 1 segundo
      setTimeout(() => {
        setIndexData({
          dax: { 
            loading: false, 
  ...
  // Función para generar datos simulados para todos los índices y períodos
  function generateMockDataForAllPeriods() {
    ...
    // Genera datos para cada índice y período
  ...
  // Función para generar datos simulados para un período específico
  function generatePeriodData(min, max, startValue, dataPoints, intervalType) {
    ...
    // Ajusta la fecha inicial según el tipo de intervalo (horario, diario, mensual)
    ...
    // Calcula el cambio y el porcentaje de cambio para el período
  ...
  // Manejador para cambiar el índice activo
  const handleIndexChange = (index) => {
    setActiveIndex(index)
  }

  // Formatea la etiqueta de tiempo para el gráfico
  const formatTime = (time) => time

  // Formatea números para mostrar dos decimales
  ...
  // Devuelve el nombre del índice según su clave
  ...
  // Renderiza la tarjeta con la información del índice activo
    ...
    return (
      <Card className="border-0 shadow-sm" style={{ backgroundColor: '#fafafa' }}>
        <CardHeader className="pb-2 relative">
          {/* Botones para cambiar entre índices */}
          ...
          {/* Título del índice y valor actual */}
